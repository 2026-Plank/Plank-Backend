...
const normalizeTeamId = (teamId) => (
  teamId === '' || teamId === undefined || teamId === null ? null : teamId
);
...
const findByIdAndUpdate = async (id, userId, updates) => {
  ...
  const setClauses = fields.map((field) => {
    ...
    if (field === 'teamId') return `${field} = :${field}`;
    return `${field} = :${field}`;
  });

  const normalizedUpdates = {
    ...updates,
    ...(Object.prototype.hasOwnProperty.call(updates, 'teamId') ? { teamId: normalizeTeamId(updates.teamId) } : {})
  };
